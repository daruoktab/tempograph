...
class QueryResult:
    ...
    timestamp: str = None  # ISO timestamp when query was evaluated
    iterations: int = 1  # Number of agent iterations (1 for vanilla/simple)
    sub_queries: List[str] = None  # Sub-queries used by agent
    retrieval_metadata: Dict[str, Any] = None  # Additional metadata from retrieval
...
class EvaluationSummary:
    ...
    metrics_by_difficulty: Dict[str, Dict[str, float]] = None


class AgenticEvaluator:
    """Evaluate Agentic RAG using same Graphiti init as ingest_agentic.py"""
    
    def __init__(self, setup_name: str, with_llm_judge: bool = True, judge_model: str = "gemini-2.5-pro", reuse_agentic_from: str = None):
        ...
        if reuse_agentic_from and "hybrid" in setup_name:
            try:
                ...
                print(f"✅ Loaded {len(self._cached_agentic_results)} cached agentic results from {reuse_agentic_from}")
    ...
    async def _extract_session_ids_from_facts(self, facts: List[str]) -> List[int]:
        ...
        try:
            ...
            for fact in facts[:15]:  # Limit to avoid huge queries
                ...
                fact_escaped = clean_fact.replace("'", "\\'").replace('"', '\\"')[:200]  # Truncate for safety
                ...
                if result and result.records:
                    for record in result.records:
                        episode_name = record.get('episode_name') or (record[0] if isinstance(record, (list, tuple)) else None)
                        if episode_name:
    ...
    def _calculate_hit_rate(self, retrieved_sessions: List[int], relevant_sessions: List[int]) -> float:
    ...
    def _calculate_reciprocal_rank(self, retrieved_sessions: List[int], relevant_sessions: List[int]) -> float:
    ...
    async def initialize(self):
        ...
        if self.setup_name == "gemini":
            ...
            from src.experiment_setups import SETUP_2A_AGENTIC_GEMMA
            self._setup = SETUP_2A_AGENTIC_GEMMA
        elif self.setup_name == "gemini_hybrid":
             from src.experiment_setups import SETUP_1H_HYBRID_GEMINI
             self._setup = SETUP_1H_HYBRID_GEMINI
        elif self.setup_name == "gemma_hybrid":
             from src.experiment_setups import SETUP_2H_HYBRID_GEMMA
             self._setup = SETUP_2H_HYBRID_GEMMA
        elif self.setup_name == "vanilla_gemini":
             from src.experiment_setups import SETUP_1V_VANILLA_GEMINI
             self._setup = SETUP_1V_VANILLA_GEMINI
        elif self.setup_name == "vanilla_gemma":
             from src.experiment_setups import SETUP_2V_VANILLA_GEMMA
        ...
        with open(QUERIES_PATH, 'r', encoding='utf-8') as f:
            data = json.load(f)
            self._queries = data.get('queries', data)  # Handle both dict with 'queries' key and direct list
        ...
        class Neo4jDriverShim(Neo4jDriver):
            ...
            async def execute_query(self, cypher_query_, parameters_=None, **kwargs):
                ...
                try:
                    return await super().execute_query(cypher_query_, params=sanitized_params, **kwargs)
                except Exception as e:
                    # Retry logic for connection errors
                    if any(x in str(e).lower() for x in ["sessionexpired", "connection"]):
        ...
        if self.setup_name in ("gemma", "gemma_hybrid"):
            ...
            print(f"  Sufficiency LLM: google/gemma-3-27b-it (Novita direct)")
            ...
            async def generate_response(self, messages, response_model=None, **kwargs):
                ...
                try:
                    ...
                    if hasattr(response, 'usage_metadata'):
                        ...
                        await tracker.track(usage.prompt_token_count, usage.candidates_token_count, self.config.model)
                    else:
                        await tracker.track(len(str(messages))/4, len(str(response))/4, self.config.model)
                except Exception:
                    pass
                return response
        ...
        if self._setup.embedder.provider == "huggingface":
            ...
            class TrackingGeminiEmbedder(GeminiEmbedder):
                async def create(self, input_data):
                    ...
                    try:
                        ...
                        from src.utils.cost_tracker import get_cost_tracker
                        tracker = get_cost_tracker()
                        await tracker.track(input_tokens, 0, config.gemini.embedding_model)
                    except Exception as e:
            ...
            print(f"  Using Gemini API embedder (with cost tracking)")
        ...
        if self.setup_name in ("gemini", "gemma"):
            ...
            from src.graph_client import SearchResult
            ...
            class GraphitiClientAdapter:
                ...
                async def search(self, query: str, num_results: int = 10):
                    ...
                    return [
                        SearchResult(
                            fact=r.fact,
                            score=getattr(r, 'score', 0.8),  # Default high score if missing
                ...
                async def search_with_temporal_filter(self, query: str, before=None, after=None, num_results: int = 10):
            ...
            sufficiency_client = self._novita_llm_client if self.setup_name == "gemma" else llm_client
            self._retrieval_agent = RetrievalAgent(adapter, retrieval_config, llm_client=sufficiency_client)
        ...
        if "hybrid" in self.setup_name:
            ...
            if "gemini" in self.setup_name:
                from src.experiment_setups import SETUP_1H_HYBRID_GEMINI as HYBRID_SETUP
                from src.experiment_setups import SETUP_1V_VANILLA_GEMINI as VANILLA_SETUP
            else:
                # Gemma Hybrid
                from src.experiment_setups import SETUP_2H_HYBRID_GEMMA as HYBRID_SETUP
                from src.experiment_setups import SETUP_2V_VANILLA_GEMMA as VANILLA_SETUP
            ...
            sufficiency_client = self._novita_llm_client if "gemma" in self.setup_name else llm_client
            agent = RetrievalAgent(adapter, retrieval_config, llm_client=sufficiency_client)
            ...
            print(f"✅ RetrievalAgent-based Graph Wrapper initialized ({mode} LLM sufficiency, 5-15 facts)")
    ...
    async def _evaluate_single(self, query_data: Dict) -> QueryResult:
        ...
        if "hybrid" in self.setup_name and hasattr(self, '_hybrid_retriever'):
            ...
            if query_id in self._cached_agentic_results:
                ...
                print(f"[REUSE] Using {len(cached_facts)} cached agentic facts for: {query[:30]}...")
                ...
                vanilla_chunks = [r.text for r in vanilla_result.results[:5]] if vanilla_result else []
                
                # Combine: agentic facts (prefixed) + vanilla chunks
                retrieved_facts = [f"[FACT] {f}" for f in cached_facts[:15]] + [f"[DETAIL] {c}" for c in vanilla_chunks]
                print(f"[REUSE] Combined: {len(cached_facts)} agentic + {len(vanilla_chunks)} vanilla = {len(retrieved_facts)} total")
                ...
                try:
                    ...
                    print(f"[DEBUG] Hybrid Retrieval done. Count: {len(hybrid_results)}")
                except Exception as e:
                ...
                vanilla_count = sum(1 for r in hybrid_results if r.source_type == "vanilla")
                retrieval_metadata = {
                    "source": "hybrid_fresh",
            ...
            if hasattr(self, '_retrieval_agent'):
                ...
                sub_queries = agent_result.metadata.get("plan", {}).get("search_queries", [])
                retrieval_metadata = {
                    "source": "agentic",
                    "query_type": agent_result.query_type.value if hasattr(agent_result.query_type, 'value') else str(agent_result.query_type),
                    "entities_found": agent_result.entities,
                    "confidence": agent_result.confidence,
                    "requires_multi_hop": agent_result.metadata.get("plan", {}).get("requires_multi_hop", False)
                }
                
                # Log iterations for debugging
                if agent_result.iterations > 1:
                    print(f"[DEBUG] Agent used {agent_result.iterations} iterations for: {query[:30]}...")
            else:
                ...
                retrieved_facts = [r.fact for r in search_results] if search_results else []
        ...
        reciprocal_rank = self._calculate_reciprocal_rank(retrieved_sessions, relevant_sessions)
    ...
    def _calculate_summary(self, results: List[QueryResult]) -> EvaluationSummary:
        ...
        mean_time = sum(r.retrieval_time_ms for r in results) / total if total > 0 else 0
        
        cs_results = [r.context_sufficiency for r in results if r.context_sufficiency is not None]
        mean_cs = sum(cs_results) / len(cs_results) if cs_results else None
        
        # By type
        by_type = {}
        for qtype in ['factual_recall', 'inference', 'multi_hop', 'counterfactual', 'temporal_reasoning']:
        ...
        for diff in ['easy', 'medium', 'hard']:
            ...
            if diff_results:
                by_diff[diff] = {
                    ...
                    'hit_rate': sum(r.hit_rate for r in diff_results) / len(diff_results),
                    'mrr': sum(r.reciprocal_rank for r in diff_results) / len(diff_results)
                }
    ...
    async def close(self):
        ...
        if self._graphiti and hasattr(self._graphiti, 'graph_driver'):
            try:
                await self._graphiti.graph_driver.close()
            except:
                pass
...
async def main():
    ...
    parser.add_argument("--setup", choices=["vanilla_gemini", "vanilla_gemma", "gemini", "gemma", "gemini_hybrid", "gemma_hybrid"], required=True)
    ...
    parser.add_argument("--reuse-agentic-from", type=str, default=None,
    ...
    try:
        ...
        if s.get('metrics_by_difficulty'):
            ...
            for diff, metrics in s['metrics_by_difficulty'].items():
                print(f"    {diff}: HR={metrics['hit_rate']:.3f}, MRR={metrics['mrr']:.3f} (n={metrics['count']})")
