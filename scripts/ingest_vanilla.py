...
class VanillaIngester:
    ...
    async def initialize(self):
        ...
        if self._setup.embedder.provider == "huggingface":
            logger.info(f"Using LOCAL HuggingFace embedder: {self._setup.embedder.name}")
    ...
    async def _embed_with_retry(self, text: str, max_retries: int = 10) -> List[float]:
        ...
        """
        is_local = self._setup.embedder.provider == "huggingface"
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not is_local:
                   from src.utils.cost_tracker import get_cost_tracker
                   tracker = get_cost_tracker()
                   await tracker.track_chars(len(text), self._setup.embedder.name)
    ...
    async def ingest(
        ...
        if resume:
            ...
            if checkpoint and checkpoint.status == "in_progress":
                ...
                logger.info(f"   Total documents: {self._chroma_db.count()}")
                return
        ...
        logger.info(f"Embedder: {self._setup.embedder.name}")
        logger.info(f"Collection: {self._setup.storage.collection_name}")
        logger.info(f"Unit: PER-TURN (1 doc = 1 turn)")
        logger.info(f"{'='*60}\n")
        ...
        try:
            for i in range(start_index, total_sessions):
                ...
                for turn_idx, turn in enumerate(session['turns']):
                    ...
                    # Add to ChromaDB
                    await self._chroma_db.add_document(doc)
                    docs_added += 1
            ...
            logger.info(f"Total documents (turns): {self._chroma_db.count()}")
            from src.utils.cost_tracker import get_cost_tracker
...
async def ingest_setup(setup_name: str, resume: bool = False, limit: Optional[int] = None):
...
async def main():
    ...
    if args.clear:
        from src.vectordb import get_chroma_client
        from src.experiment_setups import CHROMA_PERSIST_DIR
