...
class QueryResult:
    ...
    sub_queries: List[str] = None  # None for vanilla (no sub-queries)
    retrieval_metadata: Dict[str, Any] = None  # Additional metadata
    
    context_sufficiency: Optional[float] = None  # LLM judge score (0-1)
    context_sufficiency_details: Optional[Dict[str, float]] = None
    
    # Extra info
    top_k_texts: List[str] = None  # Top-K retrieved texts (preview)
...
class VanillaEvaluator:
    ...
    def __init__(self, setup_name: str, with_llm_judge: bool = True, judge_model: str = "gemini-2.5-pro", with_rerank: bool = True):
    ...
    async def initialize(self):
        ...
        from src.retrieval.vanilla_retriever import create_vanilla_retriever, VanillaRetriever
        ...
        if self.setup_name == "gemini" and not self.with_rerank:
            ...
            self._retriever = VanillaRetriever(chroma_db, settings=direct_settings, setup=self._setup)
        ...
        rerank_status = "enabled" if (self.setup_name != "gemini" or self.with_rerank) else "disabled"
    ...
    async def evaluate(self) -> Dict[str, Any]:
        ...
        for q in tqdm(queries, desc=f"Evaluating ({self.setup_name})"):
            ...
            # Retrieve
            result = await self._retriever.retrieve(q['query'])
            ...
            qr = QueryResult(
                ...
                top_k_texts=[r.text[:200] for r in result.results[:5]]  # Store more text for judge
            )
            results.append(qr)
    ...
    def _calculate_summary(self, results: List[QueryResult]) -> EvaluationSummary:
        ...
        cs_results = [r for r in results if r.context_sufficiency is not None and r.context_sufficiency > 0]
        mean_cs = sum(r.context_sufficiency for r in cs_results) / len(cs_results) if cs_results else None
...
async def evaluate_setup(setup_name: str, with_llm_judge: bool = True, judge_model: str = "gemini-2.5-pro", with_rerank: bool = True) -> Dict[str, Any]:
    """Evaluate a single setup"""
    evaluator = VanillaEvaluator(setup_name, with_llm_judge=with_llm_judge, judge_model=judge_model, with_rerank=with_rerank)
...
async def main():
    ...
    print("="*60)
    print(f"LLM Judge: {'Enabled (' + args.judge_model + ')' if with_llm_judge else 'Disabled'}")
    ...
    if args.setup in ["gemini", "all"]:
        ...
        results['vanilla_gemini'] = await evaluate_setup("gemini", with_llm_judge, args.judge_model, with_rerank)
    ...
    if args.setup in ["gemma", "all"]:
        ...
        results['vanilla_gemma'] = await evaluate_setup("gemma", with_llm_judge, args.judge_model, with_rerank)
    ...
    for name, data in results.items():
        ...
        for qtype, metrics in s['metrics_by_type'].items():
            print(f"    {qtype}: HR={metrics['hit_rate']:.3f}, MRR={metrics['mrr']:.3f} (n={metrics['count']})")
