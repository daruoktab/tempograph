...
def load_test_texts(dataset_path: str, num_samples: int = 100) -> List[str]:
    ...
    for session in dataset.get("sessions", []):
        for turn in session.get("turns", []):
            ...
            texts.append(
                turn.get("assistant_response", "")[:500]
...
async def evaluate_retrieval_quality(
    ...
    relevance_labels: List[
        List[int]
    ],  # For each query, list of relevant passage indices
) -> Dict[str, float]:
    """
    ...
    for i, q_emb in enumerate(query_embeddings):
        ...
        similarities = [cosine_similarity(q_emb, p_emb) for p_emb in passage_embeddings]
...
async def run_benchmark(
    ...
    for name in embedder_names:
        ...
        try:
            ...
            result = {**bench_result, "metrics": metrics, "status": "success"}

            logger.info(
                f"✅ {name}: {bench_result['avg_latency_ms']:.2f}ms avg, "
...
def print_comparison_table(results: List[Dict[str, Any]]):
    ...
    for r in results:
        if r.get("status") == "success":
            print(
                f"{r['model_name']:<40} {r['dimension']:>6} "
                f"{r['avg_latency_ms']:>10.2f}ms "
                f"{r['throughput_texts_per_sec']:>10.1f}/s"
            )
        else:
            print(
                f"{r['model_name']:<40} {'ERROR':<6} {r.get('error', 'Unknown')[:30]}"
            )
...
async def main():
    ...
    results = await run_benchmark(
        embedder_names=embedders_to_test, test_texts=test_texts
    )
