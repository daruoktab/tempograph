...
class LLMJudgeEvaluator:
    ...
    async def evaluate(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Evaluating {len(results_list)} queries with LLM judge ({self.judge_model})..."
        )
        ...
        summary = {
            ...
            # LLM Judge metrics
            "mean_context_sufficiency": sum(
                r["context_sufficiency_score"] for r in valid_results
            )
            / len(valid_results)
            if valid_results
            else 0,
            # Detailed breakdowns (averages)
            "mean_information_presence": sum(
                r["context_sufficiency_details"].get("information_presence", 0)
            ...
            "mean_completeness": sum(
                r["context_sufficiency_details"].get("completeness", 0)
                for r in valid_results
            )
            / len(valid_results)
            if valid_results
            else 0,
            "mean_temporal_info": sum(
                r["context_sufficiency_details"].get("temporal_info", 0)
                for r in valid_results
            )
            / len(valid_results)
            if valid_results
            else 0,
            "mean_no_contradiction": sum(
                r["context_sufficiency_details"].get("no_contradiction", 0)
                for r in valid_results
            )
            / len(valid_results)
            if valid_results
...
async def main():
    ...
    parser.add_argument(
        "--setup",
        choices=["vanilla_gemini", "vanilla_gemma", "all"],
        default="all",
        help="Setup to evaluate",
    )
    parser.add_argument(
        "--judge-model",
