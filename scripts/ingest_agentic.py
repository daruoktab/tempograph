...
for logger_name in ["", "neo4j", "neo4j.io", "neo4j.pool", "graphiti_core", "graphiti_core.driver"]:
        ...
        try:
            async with self._lock:
                ...
                if self.tokens_used + estimated_tokens > self.TOKENS_PER_MINUTE:
                    ...
                    if wait_time > 0:
                        self.wait_count += 1
                        logger.info(f"⏸️  Token budget: {self.tokens_used}/{self.TOKENS_PER_MINUTE} used, waiting {wait_time:.0f}s for window reset")
...
class RateLimitHandler:
    ...
    async def proactive_throttle(self):
        ...
        if usage_pct > 80:
            ...
            if time_to_wait > 0:
                ...
                logger.info(f"⏸️  Proactive throttle: {usage_pct:.0f}% used, waiting {time_to_wait:.0f}s for window reset")
    ...
    async def wait_on_rate_limit(self, error_msg: str = ""):
        ...
        logger.warning(f"   Tokens used: {self.tokens_used_in_window}, Error: {error_msg[:100]}...")
...
class AgenticIngester:
    ...
    async def initialize(self):
        ...
        from src.experiment_setups import (
        ...
        logger.info(f"  LLM Extraction: {self._setup.llm_extraction.name}")
...
""",
        ...
        summarize_nodes_module.versions['summary_description'] = patched_summary_description
...
""",
        ...
        prompt_library.extract_nodes.extract_message.func = patched_extract_message
        prompt_library.extract_nodes.reflexion.func = patched_extract_nodes_reflexion
        prompt_library.extract_nodes.extract_summary.func = patched_extract_summary
        
        # Patch extract_edges wrappers
        prompt_library.extract_edges.edge.func = patched_extract_edge
        prompt_library.extract_edges.reflexion.func = patched_extract_edges_reflexion
        
        # Patch summarize_nodes wrappers
        prompt_library.summarize_nodes.summarize_pair.func = patched_summarize_pair
        prompt_library.summarize_nodes.summarize_context.func = patched_summarize_context
        prompt_library.summarize_nodes.summary_description.func = patched_summary_description
        
        # Patch dedupe_nodes wrappers
        prompt_library.dedupe_nodes.node.func = patched_dedupe_node
        prompt_library.dedupe_nodes.nodes.func = patched_dedupe_nodes
        
        # Patch dedupe_edges wrappers
        prompt_library.dedupe_edges.edge.func = patched_dedupe_edge
        prompt_library.dedupe_edges.resolve_edge.func = patched_resolve_edge
        
        # Patch invalidate_edges wrappers
        prompt_library.invalidate_edges.v2.func = patched_invalidate_v2
        
        # Patch extract_edge_dates wrappers
        prompt_library.extract_edge_dates.v1.func = patched_extract_dates_v1
        
        logger.info("  ✅ Patched ALL graphiti prompts to Indonesian (8 modules + 14 wrappers)")
        ...
        class Neo4jDriverShim(Neo4jDriver):
            ...
            async def execute_query(self, cypher_query_, parameters_=None, **kwargs):
                ...
                params = parameters_ or params_from_kwargs or parameters_from_kwargs or {}
                ...
                try:
                    ...
                    with open(log_file, "a", encoding="utf-8") as f:
                        ...
                        for k, v in params.items():
                            ...
                            if "vector" in k or (isinstance(v, list) and len(v) > 100):
                                ...
                                if isinstance(v, list) and len(v) > 0:
                                     first_elem = v[0]
                                     f.write(f"    Sample[0]: {first_elem} (Type: {type(first_elem)})\n")
                                     
                                     # CASE 1: NESTED LIST (List[List[float]]) -> Flatten
                                     if isinstance(first_elem, list):
                                         f.write(f"    ⚠️ DETECTED NESTED LIST. FLATTENING...\n")
                                         if len(v) == 1:
                                             sanitized_params[k] = v[0]
                                             updates_made = True
                                             f.write(f"    ✅ FIXED: Flattened single-item nested list.\n")
                                         else:
                                             ...
                                             f.write(f"    ⚠️ WARNING: Nested list len={len(v)}. Taking v[0].\n")
                                             sanitized_params[k] = v[0]
                                             updates_made = True
                                     ...
                                     try:
                                         ...
                                         current_vec = sanitized_params[k] if updates_made else v
                                         ...
                                         if has_nan:
                                              f.write(f"    🚨 ERROR: VECTOR CONTAINS NaN/Inf! ZEROING OUT...\n")
                                              sanitized_params[k] = [0.0] * len(current_vec)
                                              updates_made = True
                                         
                                     except Exception as e:
                                         f.write(f"    ⚠️ Error checking vector content: {e}\n")
                        ...
                        if updates_made:
                            ...
                            f.write(f"✅ SANITIZATION APPLIED.\n")
                        else:
                            f.write(f"No sanitization needed.\n")
                ...
                try:
                    ...
                    return await super().execute_query(cypher_query_, params=params, **kwargs)
                    ...
                    connection_errors = [
                        "sessionexpired", "defunct connection", "semaphore timeout",
                    ...
                    if is_connection_error:
                        ...
                        for retry_num in range(max_retries):
                            logger.warning(f"⚠️ Neo4j connection error (attempt {retry_num+1}/{max_retries}): {str(conn_error)[:80]}...")
        ...
        class TrackingGeminiClient(GeminiClient):
            async def generate_response(self, messages, response_model=None, **kwargs):
                ...
                if isinstance(messages, list):
                    for m in messages:
                        if hasattr(m, 'content'): input_len += len(str(m.content)) 
                        elif isinstance(m, dict): input_len += len(str(m.get('content','')))
                        else: input_len += len(str(m))
                else:
                    input_len = len(str(messages))
                
                # Call original
                response = await super().generate_response(messages, response_model, **kwargs)
                ...
                try:
                    ...
                    await tracker.track(input_tokens, output_tokens, self.config.model)
                except Exception as e:
        ...
        if self.setup_name == "gemini":
            ...
            llm_config = LLMConfig(
                ...
                model=self._setup.llm_extraction.name,
            ...
            llm_client.small_model = self._setup.llm_extraction.name
            logger.info(f"  Forced small_model = {self._setup.llm_extraction.name} (prevents untracked flash-lite)")
        else:
            ...
            if not novita_api_key:
                raise ValueError("NOVITAAI_API_KEY environment variable required for Gemma setup")
            ...
            class NovitaLLMClient(LLMClient):
                ...
                async def generate_response(self, messages, response_model=None, **kwargs):
                    ...
                    if response_model and hasattr(response_model, 'model_json_schema') and openai_messages:
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            try:
                                from src.utils.cost_tracker import get_cost_tracker
                                tracker = get_cost_tracker()
                                input_tokens = response.usage.prompt_tokens if response.usage else int(input_len / 4)
                                output_tokens = response.usage.completion_tokens if response.usage else int(len(content) / 4)
                                await tracker.track(input_tokens, output_tokens, self.config.model)
                            except Exception as e:
                                logger.debug(f"Cost tracking failed: {e}")
                            ...
                            if response_model:
                                ...
                                try:
                                    ...
                                    if isinstance(parsed, list):
                                        fields = list(response_model.model_fields.keys()) if hasattr(response_model, 'model_fields') else ['items']
                                    ...
                                    try:
                                        ...
                                        logger.warning(f"Pydantic validation failed (attempt {attempt+1}): {str(val_error)[:100]}")
                                        if attempt < max_retries - 1:
                                            continue
                                        return parsed  # Return anyway on last attempt
                                        
                                except json.JSONDecodeError as je:
                                    logger.warning(f"JSON parse failed (attempt {attempt+1}): {str(je)[:50]}")
                                    if attempt < max_retries - 1:
                                        continue
                            ...
                            logger.error(f"Novita API error (attempt {attempt+1}): {e}")
                            if attempt == max_retries - 1:
                                raise
            ...
            llm_config = LLMConfig(
                ...
                model=self._setup.llm_extraction.name,  # google/gemma-3-27b-it
                base_url="https://api.novita.ai/openai",
            )
            llm_client = NovitaLLMClient(config=llm_config)
            logger.info(f"  Using Novita AI (no structured outputs): {self._setup.llm_extraction.name}")
        ...
        if self._setup.embedder.provider == "huggingface":
            ...
            class SentenceTransformerEmbedderShim(EmbedderClient):
                ...
                async def create_batch(self, input_data) -> Any:
                ...
                async def create(self, input_data):
                    ...
                    try:
                        if isinstance(input_data, str):
                            ...
                            total_chars = sum(len(s) for s in input_data)
                        ...
                        async with _cost_tracker._lock:
                            ...
                            _cost_tracker.model_stats["gemini-embedding-001"]["input_tokens"] += input_tokens
                            _cost_tracker.model_stats["gemini-embedding-001"]["cost_usd"] += cost
                            _cost_tracker.model_stats["gemini-embedding-001"]["call_count"] += 1
                    except Exception as e:
            ...
            embedder = TrackingGeminiEmbedder(
                config=embed_config
            )
            logger.info(f"  Using Gemini API embedder (with cost tracking)")
    ...
    async def _add_episode_with_retry(
        ...
        async def _heartbeat_logger(session_name: str, interval: int = 60):
            ...
            while True:
                ...
                elapsed += interval
                mins = elapsed // 60
                logger.warning(f"⏳ Still processing {session_name}... ({mins} min elapsed)")
                if elapsed >= timeout_minutes * 60 - interval:
                    logger.error(f"🚨 {session_name} taking too long! (>{timeout_minutes} min). May be stuck.")
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"⏰ TIMEOUT: {source} exceeded {timeout_minutes} minutes!")
                logger.error(f"   This usually means Neo4j connection died or Gemini API is not responding.")
                ...
                if any(x in error_str for x in ['rate', 'limit', '429', 'quota', 'resource_exhausted']):
        ...
        raise Exception(f"Failed to add episode after {max_retries} retries due to rate limits/timeouts")
    ...
    async def ingest(
        ...
        if resume:
            ...
            if checkpoint and checkpoint.status in ["in_progress", "failed", "batch_done"]:
                ...
                logger.info(f"   Processed: {checkpoint.processed_sessions}/{checkpoint.total_sessions}")
        ...
        # Process sessions
        logger.info(f"\n{'='*60}")
        logger.info(f"INGESTING TO {self._setup.name.upper()}")
        logger.info(f"{'='*60}")
        logger.info(f"Total sessions: {total_sessions}")
        logger.info(f"Starting from: {start_index}")
        if batch_size:
            logger.info(f"📦 BATCH: Processing {batch_sessions} sessions (until index {end_index-1})")
        logger.info(f"Group ID: {self._group_id}")
        logger.info(f"LLM Extraction: {self._setup.llm_extraction.name}")
        logger.info(f"Embedder: {self._setup.embedder.name}")
        logger.info(f"Unit: PER-SESSION (1 episode = 1 session)")
        logger.info(f"{'='*60}\n")
        
        pbar = tqdm(
            total=end_index,
            initial=start_index,
            desc=f"Ingesting ({self.setup_name})"
        )
        ...
        try:
            for i in range(start_index, end_index):
                ...
                try:
                    reference_time = datetime.fromisoformat(timestamp_str.replace('Z', '+00:00'))
                except:
                ...
                pbar.set_description(f"Ingesting ({self.setup_name}) {get_cost_tracker().get_summary()}")
                pbar.update(1)
            ...
            if end_index >= total_sessions:
                ...
                logger.info(f"{'='*60}")
                logger.info(f"Setup: {self._setup.name}")
                logger.info(f"Group ID: {self._group_id}")
                ...
                logger.info(f"Facts: {stats.get('facts', 0)}")
                logger.info(f"Episodes: {stats.get('episodes', 0)}")
                logger.info(f"Proactive throttles: {self.rate_limiter.total_proactive_waits}")
                logger.info(f"Rate limit hits: {self.rate_limiter.total_rate_limit_waits}")
                logger.info(f"FINAL COST ESTIMATE:\n{get_cost_tracker().get_detailed_summary()}")
                logger.info(f"{'='*60}\n")
            else:
                ...
                logger.info(f"{'='*60}")
                logger.info(f"Processed: {checkpoint.processed_sessions}/{total_sessions} sessions")
                ...
                logger.info(f"{'='*60}")
                logger.info(f"\n🔄 To continue, run:")
                logger.info(f"   python scripts/ingest_agentic.py --setup {self.setup_name} --resume --batch {batch_size or 5}")
                logger.info(f"{'='*60}\n")
            ...
            def extract_count(result):
                ...
                try:
                    ...
                    if hasattr(result, 'records') and len(result.records) > 0:
                        ...
                        return record.get('count', 0) if hasattr(record, 'get') else record['count']
...
async def ingest_setup(setup_name: str, resume: bool = False, batch_size: int = 5, limit: Optional[int] = None):
...
async def main():
    ...
    if args.batch:
        ...
        logger.info(f"   Use --resume to continue from last checkpoint\n")
    
    # Clear if requested
    if args.clear:
        from src.experiment_setups import NEO4J_GROUP_IDS, SetupType
    ...
    if args.setup == "all":
        ...
        await ingest_setup("gemini", resume=args.resume, batch_size=args.batch, limit=args.limit)
        await ingest_setup("gemma", resume=args.resume, batch_size=args.batch, limit=args.limit)
    else:
        await ingest_setup(args.setup, resume=args.resume, batch_size=args.batch, limit=args.limit)
    ...
    logger.info("Use 'python scripts/clear_database.py' to view/manage database status.")
