...
class GeminiProvider(BaseLLMProvider):
    ...
    async def _generate_impl(
        ...
        **kwargs,
        ...
        if system_instruction:
            ...
            response = await loop.run_in_executor(
                ...
                lambda: model.generate_content(
                    gemini_messages,
                    generation_config=gen_config,  # type: ignore[invalid-argument-type]
                ),
            )
        else:
            response = await loop.run_in_executor(
                None,
                lambda: self._client.generate_content(  # type: ignore[possibly-missing-attribute]
                    gemini_messages,
                    generation_config=gen_config,  # type: ignore[invalid-argument-type]
                ),
            )
        ...
        # Calculate cost
        pricing = self.PRICING.get(self.model, {"input": 0.0, "output": 0.0})  # type: ignore[unresolved-attribute]
        cost = (
