...
def context_recall(
    retrieved_facts: List[str], required_facts: List[str], use_semantic: bool = False
) -> MetricResult:
    """
...
def context_precision(
    retrieved_facts: List[str], relevant_facts: List[str]
) -> MetricResult:
    """
...
def temporal_precision(
    predicted_order: List[str], actual_order: List[str]
) -> MetricResult:
    """
    ...
    pred_positions = {
        item: i for i, item in enumerate(predicted_order) if item in common
    }
    actual_positions = {
        item: i for i, item in enumerate(actual_order) if item in common
    }
...
def temporal_recall(
    ...
    return MetricResult(
        name="temporal_recall",
        score=score,
        details={"matched": matched, "total_temporal": len(ground_truth_temporal)},
    )
...
def fact_coverage(
    ...
    if not retrieved_context:
        return MetricResult(
            name="fact_coverage", score=0.0, details={"message": "No context retrieved"}
        )
...
def hit_rate(
    retrieved_facts: List[str], expected_answer: str, threshold: float = 0.5
) -> MetricResult:
    """
    ...
    if not expected_words:
        return MetricResult(
            name="hit_rate", score=1.0, details={"message": "Empty expected answer"}
        )
...
def mrr(
    ...
    if not expected_words:
        return MetricResult(
            name="mrr", score=1.0, details={"message": "Empty expected answer"}
        )
...
async def context_sufficiency_llm_judge(
    ...
    try:
        ...
        return context_sufficiency_simple(
            retrieved_context=retrieved_context, expected_answer=expected_answer
        )


def context_sufficiency_simple(
    retrieved_context: str, expected_answer: str
) -> MetricResult:
    """
    ...
    if not retrieved_context or not expected_answer:
        return MetricResult(
            ...
            score=0.0,
            details={
                "message": "Missing context or expected answer",
                "method": "word_overlap",
            },
        )
...
async def calculate_all_metrics(
    ...
    results["context_recall"] = context_recall(
        retrieved_facts=retrieved_facts, required_facts=required_facts
    )
    ...
    results["context_precision"] = context_precision(
        retrieved_facts=retrieved_facts, relevant_facts=relevant_facts
    )

    # 3. Hit Rate (binary: is answer in context?)
    if expected_answer:
        results["hit_rate"] = hit_rate(
            retrieved_facts=retrieved_facts, expected_answer=expected_answer
        )
    ...
    results["fact_coverage"] = fact_coverage(
        retrieved_context=retrieved_context, ground_truth_facts=gt_facts
    )
...
if __name__ == "__main__":
    ...
    async def test_metrics():
        ...
        cs = context_sufficiency_simple(
            retrieved_context="\n".join(retrieved), expected_answer=expected_answer
        )
