...
from ..ingestion import EpisodeIngester
from ..retrieval import RetrievalAgent
from .metrics import calculate_all_metrics, MetricResult
...
class RAGEvaluator:
    ...
    async def evaluate_turn(
        ...
        if 'answer_accuracy' in metrics:
            ...
            turn_eval.details['answer_accuracy_score'] = metrics['answer_accuracy'].score
    ...
    async def evaluate_session(
        ...
        iterator = tqdm(enumerate(turns), total=len(turns), desc=f"Session {session_id}") if show_progress else enumerate(turns)
        ...
        for session in sessions:
            ...
            if limit_turns_per_session:
                ...
                session['ground_truths'] = [
                    gt for gt in session.get('ground_truths', [])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str,
                        default="output/evaluation_results/report.json")
