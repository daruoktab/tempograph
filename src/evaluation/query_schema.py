...
class EvaluationQuery(BaseModel):
    ...
    relevant_sessions: List[int] = Field(
        ...,
        min_length=1,
        description="List session IDs yang relevan (1-100)"
    )
...
class EvaluationQuerySet(BaseModel):
    ...
    metadata: dict = Field(
        ...
        description="Metadata tentang query set"
    )
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "cross_session_percent": cross_session_count / len(self.queries) * 100 if self.queries else 0
        }
