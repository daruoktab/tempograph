...
class EpisodeIngester:
    ...
    async def ingest_dataset(
        ...
        # Build communities if the client supports it
        if hasattr(self.client, "build_communities"):
            logger.info("Building communities from graph relationships...")
            try:
                num_comm = await self.client.build_communities()
                logger.info(f"Built {num_comm} communities.")
            except Exception as e:
