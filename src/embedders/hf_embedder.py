...
class HuggingFaceEmbedder(BaseEmbedder):
    ...
    async def initialize(self):
        ...
        logger.info(
            f"HuggingFace embedder initialized: {self.model_name} (dim={self._dimension})"
        )
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.info(
                f"SentenceTransformer failed ({e}), falling back to transformers"
            )
    ...
    def list_recommended_models(cls) -> Dict[str, str]:
        ...
        return {
            name: info["description"] for name, info in cls.RECOMMENDED_MODELS.items()
        }
