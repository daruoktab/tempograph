...
class RateLimiter:
    ...
    def _get_state(self, model_name: str) -> RateLimiterState:
        ...
        if model_name not in self._states:
            ...
            from ..config.settings import get_rate_limit
    ...
    async def wait_if_needed(
        self, model_name: str, estimated_tokens: int = 1000
    ) -> float:
        """
    ...
    def record_success(self, model_name: str, actual_tokens: int = 0):
        ...
        if state.consecutive_rate_limits > 0:
            ...
            state.current_delay = max(
                state.min_delay, state.current_delay / self._backoff_multiplier
            )
    ...
    def record_rate_limit_error(self, model_name: str):
        ...
        state.current_delay = min(
            self._max_retry_delay, state.current_delay * self._backoff_multiplier
        )
