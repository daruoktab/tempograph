...
class CostTracker:
    ...
    def set_log_file(self, path: str, setup_name: str = None):
        """Set file path for logging each API call to JSONL"""
        self.log_file_path = Path(path)
        self.log_file_path.parent.mkdir(parents=True, exist_ok=True)
        if setup_name:
            self.setup_name = setup_name
        logger.info(f"💾 Cost logging enabled: {self.log_file_path} (setup: {self.setup_name})")
    
    def _save_entry(self, model: str, input_tokens: int, output_tokens: int, cost: float, call_type: str = "llm"):
    ...
    def get_detailed_summary(self) -> str:
        ...
        lines.append(f"   Input: {int(self.total_input_tokens):,} tokens | Output: {int(self.total_output_tokens):,} tokens")
        lines.append("")
        lines.append("Per-Model Breakdown:")
        
        for model, stats in sorted(self.model_stats.items()):
            lines.append(f"  {model}:")
            lines.append(f"    Calls: {stats['call_count']} | Cost: ${stats['cost_usd']:.4f}")
            lines.append(f"    In: {int(stats['input_tokens']):,} | Out: {int(stats['output_tokens']):,}")
