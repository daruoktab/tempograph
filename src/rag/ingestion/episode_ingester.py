...
class EpisodeIngester:
    ...
    def parse_sessions(self, data: Dict[str, Any]) -> List[Session]:
        ...
        for session_data in data.get('sessions', []):
            ...
            try:
                ...
                logger.warning(f"Could not parse date for session {session_id}, using current time")
                base_time = datetime.now()
            
            # Parse turns
            turns = []
            ground_truths = {gt['turn_id']: gt for gt in session_data.get('ground_truths', [])}
        ...
        logger.info(f"Parsed {len(sessions)} sessions with {sum(len(s.turns) for s in sessions)} total turns")
        return sessions
    ...
    async def ingest_turn(self, turn: Turn) -> bool:
        ...
        try:
            ...
            source_desc = f"Session {turn.session_id} Turn {turn.turn_id} Speaker {turn.speaker}"
    ...
    async def ingest_session(self, session: Session, show_progress: bool = True) -> int:
        ...
        iterator = tqdm(session.turns, desc=f"Session {session.session_id}") if show_progress else session.turns
    ...
    async def ingest_dataset(
        ...
        stats = {
            "sessions_processed": min(len(sessions), limit_sessions) if limit_sessions else len(sessions),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Ingest conversation dataset to knowledge graph")
