...
from ..experiment_setups import ExperimentSetup, RetrievalSettings
...
class VanillaRetriever:
    ...
    async def initialize(self, embedder=None):
        ...
        if self.setup and self.setup.reranker_type == "llm":
            ...
            self._llm_reranker = LLMReranker(model_name=self.setup.reranker.name)
            await self._llm_reranker.initialize()
            logger.info(f"Vanilla Retriever initialized with LLM reranker: {self.setup.reranker.name}")
        else:
    ...
    async def retrieve(self, query: str) -> VanillaRetrievalResult:
        ...
        if self.settings.similarity_threshold > 0:
            candidates = [
                ...
                if r.score >= self.settings.similarity_threshold
            ]
        ...
        if self._llm_reranker and len(candidates) > 0:
            ...
            for passage, score in reranked[:self.settings.rerank_top_k]:
                if passage in text_to_result:
                    ...
                    final_results.append(VanillaSearchResult(
                        id=result.id,
                        text=result.text,
                        score=score,
        ...
        return VanillaRetrievalResult(
            ...
            metadata={
                ...
                "reranker_type": self.setup.reranker_type if self.setup else "embedding"
            }
        )
...
async def create_vanilla_retriever(
...
async def test_vanilla_retriever():
    ...
    from ..experiment_setups import SETUP_1V_VANILLA_GEMINI
