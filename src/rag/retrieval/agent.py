...
class RetrievalAgent:
    ...
    def classify_query(self, query: str) -> QueryType:
        ...
        # Temporal indicators
        temporal_keywords = [
            'kapan', 'tanggal', 'waktu', 'sejak', 'sampai', 'selama',
            'sebelum', 'sesudah', 'setelah', 'bulan', 'tahun', 'minggu',
            'kemarin', 'besok', 'lusa', 'dulu', 'baru-baru ini'
        ]
        ...
        # Causal indicators
        causal_keywords = [
            'kenapa', 'mengapa', 'karena', 'sebab', 'akibat',
            'penyebab', 'alasan', 'sehingga', 'dampak'
        ]
        ...
        agg_keywords = [
            'berapa kali', 'berapa banyak', 'total', 'jumlah',
            'seberapa sering', 'rata-rata', 'semua', 'seluruh'
        ]
        ...
        comp_keywords = [
            'beda', 'perbedaan', 'sama', 'dibanding', 'versus',
            'lebih', 'kurang', 'mirip'
        ]
    ...
    async def _ask_llm_sufficiency(self, query: str, facts: List[SearchResult]) -> Tuple[bool, str]:
        """
...
INFO_KURANG: [jika TIDAK, sebutkan informasi apa yang masih kurang]"""
        ...
        try:
            ...
            is_sufficient = "CUKUP: YA" in response.upper() or "CUKUP:YA" in response.upper()
            ...
            logger.info(f"LLM Sufficiency: {'CUKUP' if is_sufficient else 'KURANG'} - {missing_info[:50] if missing_info else 'N/A'}")
    ...
    async def evaluate_sufficiency(
        ...
        if self.llm_client:
            is_sufficient, missing_info = await self._ask_llm_sufficiency(query, state.retrieved_facts)
    ...
    async def retrieve(self, query: str) -> RetrievalResult:
        ...
        while state.iteration < self.MAX_ITERATIONS:
            ...
            for search_query in search_queries:
                ...
                for r in results:
                    if r.fact not in existing_facts and len(state.retrieved_facts) < self.MAX_FACTS:
            ...
            if plan.requires_multi_hop and state.entities_found and len(state.retrieved_facts) < self.MAX_FACTS:
            ...
            state.is_sufficient, state.confidence, missing_info_hint = await self.evaluate_sufficiency(
                query, state, plan
            )
            
            if state.is_sufficient:
                logger.info(f"Sufficient context found at iteration {state.iteration} with {len(state.retrieved_facts)} facts")
                break
            
            logger.info(f"Iteration {state.iteration}: Not sufficient, need: {missing_info_hint[:50]}...")
